...
```{r}
#Can't figure out how to do teosinte vs maize analysis at this point
```

##Missing data and amount of heterozygosity

```{r}
count_complete <- data.frame(table(unlist(complete_join[-c(1:3,2552)])))
homozygous <- filter(count_complete, Var1 %in% c("A/A", "T/T", "C/C", "G/G"))
heterozygous <- filter(count_complete, Var1 %in% c("A/C", "A/G", "A/T", "C/G", "C/T", "G/T"))
missing <- filter(count_complete, Var1 == c("-/-"))
heterozygosity <- data.frame(Heterozygosity = c("Homo", "Hetero", "Missing"), Count = c(sum(homozygous$Freq), sum(heterozygous$Freq), missing$Freq))
ggplot(heterozygosity) +
  geom_col(aes(x=Heterozygosity, y=Count, fill=Heterozygosity), show.legend = FALSE)
#If you know a better way to do this please let me know
```
Bar graph comparing amount of heterozygous, homozygous, and missing sites. Homozygous sites are overwhelmingly dominant with heterozygous sites accounting for roughly 1/5 of the sites and missing data makes about 1/20 of the sites.
...
```
Bar graph showing rough position of SNPs on each chromosome with position being divided into 5 equal lengths. SNPs appear to be most common at the beginning and end of each chromosome.
